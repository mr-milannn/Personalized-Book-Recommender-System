...
class CFWrapper:
    def __init__(self, cf_dict):
        self.components = cf_dict["components"]
        self.user_mapping = cf_dict["user_mapping"]
        self.item_mapping = cf_dict["item_mapping"]
        self.reverse_user_mapping = cf_dict["reverse_user_mapping"]
        self.reverse_item_mapping = cf_dict["reverse_item_mapping"]
    ...
    def predict_rating(self, user_id, book_id):
        ...
        # reconstruct rating using dot product
        user_factors = self.components[:, :].T[user_idx]  # simplified
        item_factors = self.components[:, item_idx]
        return float(np.dot(user_factors, item_factors))


class CBWrapper:
    def __init__(self, cb_dict):
        self.vectorizer = cb_dict["vectorizer"]
        self.tfidf_matrix = cb_dict["tfidf_matrix"]
        self.books_df = cb_dict["books"]
    ...
    def recommend_by_tags(self, tags, n_recommendations=10):
        from sklearn.metrics.pairwise import linear_kernel
        tag_vector = self.vectorizer.transform([tags])
...
class HybridWrapper:
    def __init__(self, hybrid_dict):
        self.cf = CFWrapper(hybrid_dict["cf"])
        self.cb = CBWrapper(hybrid_dict["cb"])

    def recommend_books(self, user_id, n_recommendations=10):
    # 1. Collaborative recommendations
        cf_recs = []
        if user_id in self.cf.user_mapping:
            for book_id in list(self.cf.item_mapping.keys())[:500]:
                score = self.cf.predict_rating(user_id, book_id)
                cf_recs.append((book_id, score))
            cf_recs.sort(key=lambda x: x[1], reverse=True)
            cf_recs = cf_recs[:n_recommendations]
        else:
            return []

    # 2. Content-based boost
        rec_books = self.cb.books_df[self.cb.books_df['book_id'].isin([bid for bid, _ in cf_recs])]
        rec_books = rec_books.copy()
    
    # Map scores safely
        score_map = {bid: score for bid, score in cf_recs}
        rec_books['hybrid_score'] = rec_books['book_id'].map(score_map)
    
        return rec_books.to_dict("records")
